...
async function handleLocationChange() {
    ...
    const allCountyNodes = Object.entries(countyCitiesMap)
        ...
        .reduce(function(h, obj) {
         ...
         const sensorCoverageData = Object.keys(allCountyNodes).map(key => {
            return {
                sensor: key, 
                sensorAvg : Math.round(allCountyNodes[key].reduce((a, b) => a + (Number(b.value) || 0), 0)/allCountyNodes[key].length),
            } 
         }).filter(sensorType  => sensorType.sensor !== "timestamp" && sensorType.sensor !== "height" && sensorType.sensor !== "lon" && sensorType.sensor !== "lat")
